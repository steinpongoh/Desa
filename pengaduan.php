...
if (isset($_POST['kirim'])) {
  if (pengaduan($_POST, $_FILES) > 0) {
    echo "<script>
      ...
      document.location.href='list_pengaduan.php'
    </script>";
  } else {
    echo "<script>
      ...
      <form id="form" method="post" action="" class="col-md-8 col-10 " enctype="multipart/form-data">
        ...
        <div class="container-form shadow p-5 mt-4 mb-4 rounded-2 border-top border-success border-3 bg-light">
          ...
          <input name="status" type="hidden"
            class="form-control form-control-sm border border border-black p-2 mb-2 border-opacity-50"
            autocomplete="off" id="">
          <div class="mb-3">
            <label for="nik" class="form-label">NIK:</label>
            <input name="nik" type="number"
              class="form-control form-control-sm border border border-black p-2 mb-2 border-opacity-50"
              autocomplete="off" id="nik">
          </div>
          <div class="mb-3">
          ...
          <div class="mb-3">
            ...
            <input name="alamat" type="text"
              class="form-control form-control-sm border border-black p-2 mb-2 border-opacity-50" autocomplete="off"
              id="alamat">
          </div>
          <div class="mb-3">
          ...
          <div class="mb-3">
            ...
            <input name="tanggal" type="datetime-local"
              class="form-control form-control-sm border border-black p-2 mb-2 border-opacity-50" id="tanggal">
          </div>
          <div class="mb-3">
            <label for="gambar" class="form-label">Foto:</label>
            <input name="gambar" class="form-control form-control-sm border border-black p-2 mb-2 border-opacity-50"
              id="gambar" type="file">
          </div>
          <div class="mb-3">
            <label for="detail" class="form-label">Isi Pengaduan:</label>
            <textarea form="form" name="detail"
              class="form-control form-control-sm border border-black p-2 mb-2 border-opacity-50" autocomplete="off"
              id="detail" rows="5"></textarea>
          </div>
          <div class="mt-3">
            <button type="submit" id="kirim" name="kirim"
              class="btn btn-success d-flex justify-content-end">Kirim</button>
          </div>
        </div>
      </form>
    </div>
